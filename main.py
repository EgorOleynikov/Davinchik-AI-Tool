...
import keyboard
from characterai import aiocai, errors
...
class CAIMessage:
    ...
    async def _send_message(self):
        try:
            ...
            logger.critical("Invalid CAI token")
            print(e)
            print("Invalid CAI token")
            input("press to close...")
            sys.exit(0)
...
try:
    ...
    async def first(event):
        ...
        if event.sender_id == 1234060895 and event.message.text != '':
            ...
            if not re.match(r'.*,\s\d{2},\s', message_text):
                ...
                async def regen():
                    ...
                    match key_pressed:
                        ...
                        case _:  # default
                            logger.info("no key pressed, proceeding")
                ...
                async def response_validate(text: str, p: True | False) -> str:  # gets raw message text, parses,
                    ...
                    logger.info(f"validating response, state: {p}")
                    try:
                        ...
                        #  TODO: for some reason after invalid response breaks the chain
                        logger.info("bad format, trying again")
